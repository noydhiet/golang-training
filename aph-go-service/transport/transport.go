...
//Hello Daerah End-point

// var ErrEmpty = errors.New("empty string")

func (aphService) HelloDaerahService(_ context.Context, name string, jenis_kelamin string, asal_kota string) string {

	return call_ServiceHelloDaerah(name, jenis_kelamin, asal_kota)
}

func call_ServiceHelloDaerah(name string, jenis_kelamin string, asal_kota string) string {
...
func makeHelloDaerahEndpoint(aph AphService) endpoint.Endpoint {
	return func(ctx context.Context, request interface{}) (interface{}, error) {
		...
		logging.Log(fmt.Sprintf("Name Request %s", req.NAME))
		v := aph.HelloDaerahService(ctx, req.NAME, req.JENIS_KELAMIN, req.ASAL_KOTA)
		logging.Log(fmt.Sprintf("Response Final Message %s", v))
...
func encodeDaerahResponse(_ context.Context, w http.ResponseWriter, response interface{}) error {
	return json.NewEncoder(w).Encode(response)
}

func RegisterHttpsServicesAndStartListenerDaerah() {
	aph := aphService{}

	HelloDaerahHandler := httptransport.NewServer(
		makeHelloDaerahEndpoint(aph),
		decodeHelloDaerahRequest,
		encodeDaerahResponse,
	)
