...
import (
	...
	"golang-service/datastruct"
	"golang-service/logging"
...
func makeHelloDaerahEndpoint(aph AphService) endpoint.Endpoint {
	return func(ctx context.Context, request interface{}) (interface{}, error) {
		...
		logging.Log(fmt.Sprintf("Name Request %s", req.NAME, req.GENDER, req.KOTA))
		v := aph.HelloDaerahService(ctx, req.NAME, req.GENDER, req.KOTA)
		logging.Log(fmt.Sprintf("Response Final Message %s", v))
